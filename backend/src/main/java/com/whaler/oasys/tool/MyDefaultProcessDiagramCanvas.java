...
public class MyDefaultProcessDiagramCanvas {
    ...
    protected static final Logger LOGGER = LoggerFactory.getLogger(MyDefaultProcessDiagramCanvas.class);
    ...
    // Predefined sized
    protected static final int ARROW_WIDTH = 5*SCALE/2;
    protected static final int CONDITIONAL_INDICATOR_WIDTH = 16*SCALE;
    protected static final int DEFAULT_INDICATOR_WIDTH = 10*SCALE;
    protected static final int MARKER_WIDTH = 12*SCALE;
    protected static final int FONT_SIZE = 11*SCALE;
    protected static final int FONT_SPACING = 2*SCALE;
    protected static final int TEXT_PADDING = 3*SCALE;
    protected static final int ANNOTATION_TEXT_PADDING = 7*SCALE;
    ...
    // Strokes
    protected static final Stroke THICK_TASK_BORDER_STROKE = new BasicStroke(3.0f*Float.valueOf(SCALE));
    protected static final Stroke GATEWAY_TYPE_STROKE = new BasicStroke(3.0f*Float.valueOf(SCALE));
    protected static final Stroke END_EVENT_STROKE = new BasicStroke(3.0f*Float.valueOf(SCALE));
    protected static final Stroke MULTI_INSTANCE_STROKE = new BasicStroke(1.3f*Float.valueOf(SCALE));
    protected static final Stroke EVENT_SUBPROCESS_STROKE = new BasicStroke(1.0f*Float.valueOf(SCALE), BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER, 1.0f, new float[] { 1.0f }, 0.0f);
    protected static final Stroke NON_INTERRUPTING_EVENT_STROKE = new BasicStroke(1.0f*Float.valueOf(SCALE), BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER, 1.0f, new float[] { 4.0f, 3.0f }, 0.0f);
    protected static final Stroke HIGHLIGHT_FLOW_STROKE = new BasicStroke(1.3f*Float.valueOf(SCALE));
    protected static final Stroke ANNOTATION_STROKE = new BasicStroke(2.0f*Float.valueOf(SCALE));
    protected static final Stroke ASSOCIATION_STROKE = new BasicStroke(2.0f*Float.valueOf(SCALE), BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER, 1.0f, new float[] { 2.0f, 2.0f }, 0.0f);

    // icons
    protected static final int ICON_PADDING = 5*SCALE/2;
    ...
    */
    ...
    public void initialize(String imageType) {
        ...
        try {
            ...
            USERTASK_IMAGE = scaleImage(USERTASK_IMAGE);
            SCRIPTTASK_IMAGE = scaleImage(SCRIPTTASK_IMAGE);
            SERVICETASK_IMAGE = scaleImage(SERVICETASK_IMAGE);
            RECEIVETASK_IMAGE = scaleImage(RECEIVETASK_IMAGE);
            SENDTASK_IMAGE = scaleImage(SENDTASK_IMAGE);
            CASETASK_IMAGE = scaleImage(CASETASK_IMAGE);
            MANUALTASK_IMAGE = scaleImage(MANUALTASK_IMAGE);
            BUSINESS_RULE_TASK_IMAGE = scaleImage(BUSINESS_RULE_TASK_IMAGE);
            SHELL_TASK_IMAGE = scaleImage(SHELL_TASK_IMAGE);
            DMN_TASK_IMAGE = scaleImage(DMN_TASK_IMAGE);
            CAMEL_TASK_IMAGE = scaleImage(CAMEL_TASK_IMAGE);
            MULE_TASK_IMAGE = scaleImage(MULE_TASK_IMAGE);
            HTTP_TASK_IMAGE = scaleImage(HTTP_TASK_IMAGE);

            TIMER_IMAGE = scaleImage(TIMER_IMAGE);
            COMPENSATE_THROW_IMAGE = scaleImage(COMPENSATE_THROW_IMAGE);
            COMPENSATE_CATCH_IMAGE = scaleImage(COMPENSATE_CATCH_IMAGE);
            CONDITIONAL_CATCH_IMAGE = scaleImage(CONDITIONAL_CATCH_IMAGE);
            ERROR_THROW_IMAGE = scaleImage(ERROR_THROW_IMAGE);
            ERROR_CATCH_IMAGE = scaleImage(ERROR_CATCH_IMAGE);
            ESCALATION_THROW_IMAGE = scaleImage(ESCALATION_THROW_IMAGE);
            ESCALATION_CATCH_IMAGE = scaleImage(ESCALATION_CATCH_IMAGE);
            MESSAGE_THROW_IMAGE = scaleImage(MESSAGE_THROW_IMAGE);
            MESSAGE_CATCH_IMAGE = scaleImage(MESSAGE_CATCH_IMAGE);
            SIGNAL_THROW_IMAGE = scaleImage(SIGNAL_THROW_IMAGE);
            SIGNAL_CATCH_IMAGE = scaleImage(SIGNAL_CATCH_IMAGE);
        } catch (IOException e) {
