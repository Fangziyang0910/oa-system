...
import 'package:fixflow/component/approvalpage/historyTaskDetail.dart';
...
class HistoryApprovalListWidget extends StatefulWidget {
  const HistoryApprovalListWidget({super.key});

  @override
  State<HistoryApprovalListWidget> createState() => _HistoryApprovalListWidgetState();
}

class _HistoryApprovalListWidgetState extends State<HistoryApprovalListWidget> {
  ...
  Future<void> _fetchData({bool refresh = false}) async {
    ...
    try {
      final response = await http.get(
        Uri.parse(ApiUrls.listApprovalTasksCompleted),
        headers: {
                ...
                itemBuilder: (context, index) {
                  ...
                  return Card(
                    ...
                    child: ListTile(
                      ...
                      subtitle: Text(
                        '审批时间：' + parseDateFormat(task['endTime']),
                      ...
                      onTap: () {
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (context) => HistoryTaskDetailWidget(
                              taskId: task['taskId'],
                            ),
                          ),
                        );
